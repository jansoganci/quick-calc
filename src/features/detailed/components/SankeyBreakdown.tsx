...
/**
 * Pixel arithmetic for the supplementary Sankey
 * (DETAILED_FRONTEND_IMPLEMENTATION_SPEC.md §4.5). Visual proportion only —
 * every value it touches is already computed by `buildBreakdown`. Mirrors
 * Quick/Lite's `SankeyBreakdown` (same technique, same R7 rule); the layout
 * constants are wider because Detailed's category labels run longer
 * ("Kanal maliyetleri — ambalaj ve kurye", "Ödeme ve platform kesintileri").
 *
...
/**
 * Supplementary to the reconciliation bar (`ResultBar`), never a replacement.
 * Renders the same `BreakdownView` categories, in the same locked order and
 * the same colours, as a single-source Sankey fan-out.
 */
export function SankeyBreakdown({ breakdown, gross }: SankeyBreakdownProps) {
  ...
  return (
    <svg
      ...
      className="mt-6 block h-auto w-full lg:mt-7"
      role="img"
      aria-label="Aylık işletme sonucunun kategorilere dağılımı"
    >
      {rows.map((row, index) => {
        ...
        return (
          <path
            key={row.key}
            d={ribbonPath(sourceSeg, targetSeg)}
            style={{ fill: row.fillVar, fillOpacity: 0.38 }}
            stroke="var(--qc-rule-mid)"
            strokeWidth="1"
          />
        )
      })}
      ...
      {rows.map((row, index) => {
        ...
        return (
          <rect
            ...
            y={targetSeg.y0}
            width={NODE_WIDTH}
            height={targetSeg.y1 - targetSeg.y0}
            style={{ fill: row.fillVar }}
          />
        )
      })}
      ...
      {rows.map((row, index) => {
        ...
        return (
          <text key={row.key} x={LABEL_X} y={center} dominantBaseline="middle" fontSize="12">
            <tspan
              className="font-sans"
              style={{ fill: 'var(--qc-ink)', fontWeight: row.key === 'operatingResult' ? 600 : 400 }}
            >
              {row.label}
            </tspan>
            <tspan className="font-mono" style={{ fill: 'var(--qc-muted)' }} dx="8">
