...
/**
 * Pixel arithmetic for the supplementary Sankey (DESIGN_DIRECTION.md R7). Visual
 * proportion only — every value it touches is already computed by `buildQuickView`.
 *
 * `GAP` between target nodes is a fixed constant rather than derived from node
 * height on purpose: it is what keeps adjacent labels legible even when a
 * category is genuinely 0 (e.g. `initialCapex = 0`, so two neighbouring
...
/**
 * R7 — supplementary to the stacked bar (`StackedBar`), never a replacement.
 * Renders the same `breakdownPerSale` categories, in the same locked §9.2
 * order and the same colours, as a single-source Sankey fan-out.
 */
export function SankeyBreakdown({ bar, ticketFormatted }: SankeyBreakdownProps) {
  ...
  return (
    <svg
      ...
      role="img"
      aria-label="Ortalama satışın kategorilere dağılımı"
    >
      {bar.map((segment, index) => {
        ...
        return (
          <path
            ...
            style={{ fill: segment.color, fillOpacity: 0.38 }}
      ...
      {bar.map((segment, index) => {
        ...
        return (
          <text key={segment.key} x={LABEL_X} y={center} dominantBaseline="middle" fontSize="12">
            <tspan
              className="font-sans"
              style={{ fill: 'var(--qc-ink)', fontWeight: segment.key === 'remaining' ? 600 : 400 }}
            >
              {segment.label}
            </tspan>
            <tspan className="font-mono" style={{ fill: 'var(--qc-muted)' }} dx="8">
